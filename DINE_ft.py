...
def cal_acc(loader, netF, netB, netC, flag=False):
    ...
    accuracy = torch.sum(
        torch.squeeze(predict).float() == all_label).item() / float(
            all_label.size()[0])
    mean_ent = torch.mean(loss.Entropy(all_output)).cpu().data.item() / np.log(
        all_label.size()[0])
...
def train_target(args):
    ...
    netB.eval()
    netC.eval()
    acc_s_te, _, pry, mean_ent = cal_acc(dset_loaders['test'], netF, netB,
                                         netC, False)
    log_str = 'Task: {}, Iter:{}/{}; Accuracy={:.2f}%, Ent={:.3f}'.format(
    ...
    while iter_num < max_iter:
        ...
        if iter_num % interval_iter == 0 or iter_num == max_iter:
            ...
            netB.eval()
            netC.eval()
            acc_s_te, _, pry, mean_ent = cal_acc(dset_loaders['test'], netF,
                                                 netB, netC, False)
            log_str = 'Task: {}, Iter:{}/{}; Accuracy={:.2f}%, Ent={:.3f}'.format(
                args.name, iter_num, max_iter, acc_s_te, mean_ent)
            args.out_file.write(log_str + '\n')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dset',
                        ...
                        default='office-home',
                        choices=[
                            'VISDA-C', 'office', 'image-clef', 'office-home',
                            'office-caltech'
                        ])
    ...
    parser.add_argument('--net',
                        type=str,
                        default='resnet50',
                        help="alexnet, vgg16, resnet18, resnet50, resnext50")
    parser.add_argument(
        '--net_src',
        type=str,
        default='resnet50',
    ...
    for i in range(len(names)):
        ...
        args.output_dir = osp.join(
            args.output, args.net_src + '_' + args.net, str(args.seed),
            args.da, args.dset,
        ...
        args.out_file = open(osp.join(args.output_dir, 'log_finetune.txt'),
                             'w')
